...
height, width, h_start, w_start = int(height/4), int(width/4), int(h_start/4), int(w_start/4)
...
if __name__ == '__main__':
    for instrument_index in range(1, 9):
        ...
        for file_name in tqdm(list((train_path / instrument_folder / 'left_frames').glob('*'))):
            ...
            img = cv2.resize(img, dsize=(0,0), fx=0.25, fy=0.25, interpolation=cv2.INTER_AREA)
            ...
            for mask_folder in mask_folders:
                ...
                mask = cv2.resize(mask, dsize=(0, 0), fx=0.25, fy=0.25, interpolation=cv2.INTER_NEAREST)  # INTER_NEAREST is used instead of INTER_AREA (the typical choisce for shrinking) because the mask value is an indicator of part of instrument

...
import json
import re
#from cocoapi_master.PythonAPI.pycocotools.cocostuffhelper import pngToCocoResult
...
def pngToCocoResultDemo(dataDir='../..', resType='examples', indent=None):
    ...
    pngFolder = '/Users/vishalrao/PycharmProjects/DeepLearning/robot-surgery-segmentation/data/temp_masks/binary'
    jsonPath = '/Users/vishalrao/PycharmProjects/DeepLearning/robot-surgery-segmentation/data/temp_masks/binary.json'
    ...
    with io.open(jsonPath, 'w', encoding='utf8') as output:
        ...
        for i, imgName in zip(range(0, imgCount), imgNames):
            ...
            tokens = imgName.split('frame')
            imgId = "frame000.png".split("frame")[1].split(".")[0]
            """if len(tokens) == 1:
                # COCO 2017 format
                imgId = int(imgName)
            elif len(tokens) == 3:

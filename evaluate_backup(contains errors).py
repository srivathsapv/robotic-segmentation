...
from prepare_data import height, width, h_start, w_start, read_im
    ...
    if args.problem_type == 'binary':
        for instrument_id in tqdm(range(1, 9)):
            for file_name in (
                ...
                pred_im = read_im(file_name, is_mask=True)
                y_pred = (pred_im > 255 * 0.5).astype(np.uint8)[h_start:h_start + height,
        ...
        for instrument_id in tqdm(range(1, 9)):
            for file_name in (
                ...
                '*'):
                y_true = cv2.imread(str(file_name), 0) #read_im(file_name, is_mask=True)

                pred_file_name = Path(args.target_path) / 'parts' / (
                    'instrument_dataset_' + str(instrument_id)) / file_name.name

                y_pred = read_im(pred_file_name, is_mask=True)[h_start:h_start + height, w_start:w_start + width]
